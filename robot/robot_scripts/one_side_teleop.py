...
from constants import MASTER2PUPPET_JOINT_FN, DT, START_ARM_POSE, MASTER_GRIPPER_JOINT_MID, PUPPET_GRIPPER_JOINT_CLOSE, ROBOT_MODEL, NUM_JOINTS
...
def teleop():
    ...
    puppet_bot = InterbotixManipulatorXS(robot_model=ROBOT_MODEL, group_name="arm", gripper_name="gripper", robot_name=f'puppet', init_node=True)
    master_bot = InterbotixManipulatorXS(robot_model=ROBOT_MODEL, group_name="arm", gripper_name="gripper", robot_name=f'master', init_node=False)
    ...
    while True:
        ...
        master_state_joints = master_bot.dxl.joint_states.position[:NUM_JOINTS]
        puppet_bot.arm.set_joint_positions(master_state_joints, blocking=False)
        # sync gripper positions
        master_gripper_joint = master_bot.dxl.joint_states.position[NUM_JOINTS]
...
def master_debug():
    """ A standalone function for experimenting arm. No data recording. Useful for calibration. """
    master_bot = InterbotixManipulatorXS(robot_model=ROBOT_MODEL, group_name="arm", gripper_name="gripper", robot_name=f'master', init_node=True)

    prep_robots(master_bot, None)
    press_to_start(master_bot)
    print("DONE!!")
    return
    ...
    while True:
        # sync joint positions
        master_state_joints = master_bot.dxl.joint_states.position[:NUM_JOINTS]
        gripper_joint = master_bot.gripper.core.joint_states.position[NUM_JOINTS]
        #print(f"gripper joint pos={gripper_joint}")
        # sync gripper positions
        master_gripper_joint = master_bot.dxl.joint_states.position[NUM_JOINTS]
        puppet_gripper_joint_target = MASTER2PUPPET_JOINT_FN(master_gripper_joint)
        # sleep DT
