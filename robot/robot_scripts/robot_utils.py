import numpy as np
import time
from constants import DT, NUM_JOINTS
...
class Recorder:
    def __init__(self, name, init_node=True, is_debug=False):
        from collections import deque
        import rospy
        ...
        rospy.Subscriber(f"/puppet/joint_states", JointState, self.puppet_state_cb)
        rospy.Subscriber(f"/puppet/commands/joint_group", JointGroupCommand, self.puppet_arm_commands_cb)
        rospy.Subscriber(f"/puppet/commands/joint_single", JointSingleCommand, self.puppet_gripper_commands_cb)
        if self.is_debug:
...
def get_arm_joint_positions(bot):
    return bot.arm.core.joint_states.position[:NUM_JOINTS]

def get_arm_gripper_positions(bot):
    joint_position = bot.gripper.core.joint_states.position[NUM_JOINTS]
